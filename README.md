...
Подписки и токены хранятся в общей базе SQLite `bot/user_data.db` и разделяются по `user_id`, поэтому
...
При первом запуске будет создан файл `bot/user_data.db` для хранения подписок и токенов.
